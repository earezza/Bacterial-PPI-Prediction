...
class OneToAll(object):
    def __init__(self, df_scores, df_labels, proteinID, sens=5, deg=7, on=True):
        ...
        try:
            ...
            if self.knee.y[0 if self.knee.knee is None else self.knee.knee] == 0:
                self.knee.knee = np.where(self.knee.y == 0 if type(min(np.ma.masked_equal(self.knee.y, 0))) is np.ma.core.MaskedConstant else min(np.ma.masked_equal(self.knee.y, 0)))[0][0]
                self.knee.knee_y = 0 if type(min(np.ma.masked_equal(self.knee.y, 0))) is np.ma.core.MaskedConstant else min(np.ma.masked_equal(self.knee.y, 0))
            if self.elbow.y[0 if self.elbow.knee is None else self.elbow.knee] == 0:
                self.elbow.knee = np.where(self.elbow.y == 0 if type(min(np.ma.masked_equal(self.elbow.y, 0))) is np.ma.core.MaskedConstant else min(np.ma.masked_equal(self.elbow.y, 0)))[0][0]
                self.elbow.knee_y = 0 if type(min(np.ma.masked_equal(self.elbow.y, 0))) is np.ma.core.MaskedConstant else min(np.ma.masked_equal(self.elbow.y, 0))
        except:
            if not self.scores[self.scores.columns[-1]].any():
                self.knee = No_Knee(self.scores[self.scores.columns[-1]])
                self.elbow = No_Knee(self.scores[self.scores.columns[-1]])
                self.knee.knee = 0
                self.knee.knee_y = 0
                self.elbow.knee = 0
                self.elbow.knee_y = 0
            ...
            # When all scores are zero (likely for SPRINT), no knee/elbow possible, set values to 0
            # Set self-interaction score ~ 0 in scores array to enable KneeLocator()
            scores = self.scores.copy()
            scores.iloc[0][scores.columns[-1]] = 1e-10
...
def create_RP_traintest_sets(cv_subset_dir_path, prediction_results_dir_path, labels, result_path):
    ...
    pred_files = [ x for x in prediction_files if 'prediction' in x and ('train' in x or 'fold' in x) ]
    pred_files.sort()
    ...
    # For each k-fold subset, extract RP features for train and test subsets
    for i in range(0, len(train_files)):
        print('==== FOLD %s ===='%i)
        # Read trained PPI subset
        trained = read_df(cv_subset_dir_path + train_files[i])
        # Get PPIs for testing subset
        tested = remove_matching_pairs(labels, trained)
        tested[tested.columns[-1]] = tested[tested.columns[-1]].astype(int)
        ...
        # Get RP datasets
        print('Getting RP training dataset for fold %s'%i)
        rp_train = create_RP_dataset(predictions, trained)
        print('Getting RP testing dataset for fold %s'%i)
        rp_test = create_RP_dataset(predictions, tested)
        
        # Save to file
        rp_train.to_csv(result_path + 'RP_' + train_files[i], sep='\t', index=False)
        rp_test.to_csv(result_path + 'RP_' + train_files[i].replace('train', 'test'), sep='\t', index=False)
...
if __name__ == '__main__':
    ...
    # If cross-validation subsets to be made
    if args.trained != None:
        create_RP_traintest_sets(args.trained, args.predictions, labels, args.results)
        exit()
    # If trained on single training file
    else:
        all_predictions = pd.read_csv(args.predictions, delim_whitespace=True, header=None)
